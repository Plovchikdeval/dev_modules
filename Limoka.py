...
class LimokaAPI:
    async def get_all_modules(self) -> list:
        async with aiohttp.ClientSession() as session:
            async with session.get('https://ksheta.top/api/module/all') as response:
    ...
    async def get_module_by_id(self, id) -> dict:
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://ksheta.top/api/module/{id}') as response:
                return await response.json()
            
    async def get_module_raw(self, developer, module_name) -> str:
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://ksheta.top/api/module/{developer}/{module_name}') as response:
...
class Limoka(loader.Module):
    ...
    @loader.command()
    async def devtest(self, message: Message):
        """ [query] - Search module"""
        ...
        if module_id == 0:
            ...
            link = f"https://ksheta.top/api/module/download/{module_id}"
    ...
    @loader.watcher(only_messages=True, startswith="#install", from_id=7930362801)
    ...
    @loader.watcher(only_messages=True, from_id=7930362801)
